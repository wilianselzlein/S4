    ...
    async def middleware_handler(request):
        try:
            ...
            return web.json_response(
                {
                    "msg": "Erro interno. Tente novamente ou entre em contato com nossa equipe."
                },
                status=500,
            )
...
async def avaliar(request):
    ...
    relacionados, severidades, tempos, pessoas, texto = _avaliar.portal(
        atendimento + "/" + item
    )
...
async def index(request):
    ...
    dsl = {"sugeridos": sugeridos, "avaliados": avaliados}
...
async def buscas(request):
    ...
    ultimas = []
    ultimas = es.search(
        index=config.elasticsearch_search,
        body={
            "size": 100,
...
async def searchs(request):
    ...
    for result in results["hits"]["hits"]:
        ...
        try:
            ...
            card["label"] = (
                str(int(result["_source"]["atendimento"]))
                + "/"
                + str(int(card["item"]))
            )
        except:
