...
class Texto(object):
    ...
    def RemovePadroes(self, s):
        ...
        t = re.sub(
            r"dados basico.*servidor*.*aplicacao.*usuario.*login.*contato", " ", t
        )
    ...
    def __init__(self):
        ...
        #
        self.re_punkts = re.compile(
            r"(\w+)([%s])([ %s])" % (punctuations, punctuations), re.UNICODE
        )
        self.re_punkts_b = re.compile(
