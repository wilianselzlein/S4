...
class Postgres(PostgresBase.Base):
    ...
    @staticmethod
    def inserir(self, sac, item, texto, tratado, stemming, severidade, tempo, data, encerramento):
        # tempo = str(tempo).replace(".", ",")
        sql = "insert into sac (atendimento, item, original, texto, stemming, severidade, tempo, data, encerramento) " \
              "values (" + str(sac) + ", " + str(item) + ", '" + texto + "', '" + tratado + "', '" \
              + stemming + "', " + str(severidade) + ", " + str(tempo) + ", '" + str(data) + "', '" + str(encerramento) + "') " \
